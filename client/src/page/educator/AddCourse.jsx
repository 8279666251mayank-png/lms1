...
useEffect(()=>{
  ...
  return (
    ...
    <div className='flex items-center justify-between flex-wrap'>
      <div className='flex flex-col gap-1'>
       <p>Course Price</p>
       <input onChange={e =>setCoursePrice(e.target.value)} value={coursePrice} type="number" placeholder='0' className='outline-none md:py-2.5 w-28 px-3 rounded border border-gray-500 ' required></input>
      </div>
     ...
     <div>
      {chapters.map((chapter,chapterIndex)=>(
        <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
          ...
          <span className='font-semibold'>{chapterIndex+1}{chapter.chapTitle}</span>
          </div>
...
<img src={assets.cross_icon} alt="" onClick={()=>handleLecture("remove",chapter.chapterId,lectureIndex)} className='cursor-pointer'/>
      ...
      {showPopup &&  (
        <div className='fixed inset-0 flex items-center justify-center bg-gray-800/90 '>
         <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
          ...
          <div className='flex gap-2 my-4'>
           ...
           checked={lectureDetails.isPreviewFree} onChange={(e)=>setLectureDetails({...lectureDetails,isPreviewFree:e.target.checked})}/>
          </div>
